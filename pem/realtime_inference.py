...
class PEMNetWrapper:
    def __init__(self, config_path, checkpoint_path, cad_path, device="cuda"):
        ...
        # 3. Load CAD Model Geometry (needed for radius/scale calculations)
        print(f"Loading CAD Mesh from: {cad_path}")
    ...
    def load_templates(self, template_dir):
        ...
        print("Template features successfully cached in VRAM.")

    @torch.inference_mode()
    def estimate_pose(self, rgb_frame, depth_frame, mask, K_matrix):
        """
        ...
        # Sample points to match network requirements
        n_samples = self.cfg.test_dataset.n_sample_observed_point
        replace = len(choose) <= n_samples
        choose_idx = np.random.choice(np.arange(len(choose)), n_samples, replace=replace)
        ...
        pred_R = out['pred_R'][0].cpu().numpy()
        pred_t = out['pred_t'][0].cpu().numpy() * 1000.0 # Convert back to mm
