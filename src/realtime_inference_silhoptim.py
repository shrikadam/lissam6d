...
def get_rgb_centroid(mask, Z_guess, K_matrix):
    """Back-projects the 2D mask centroid to 3D using the template's Z distance."""
    ys, xs = np.where(mask > 0)
    if len(ys) == 0: return None
    
    # Use median to avoid outliers from occlusion shapes
    u = np.median(xs)
    v = np.median(ys)
    ...
    return np.array([[X], [Y], [Z_guess]], dtype=np.float32)

def draw_6d_pose(frame, K, R, t, bbox_3d):
    """
    Projects a 3D bounding box, coordinate axes, and origin centroid onto the 2D frame.
    """
    ...
    # ---> NEW: Draw the detected position centroid
    cv2.circle(frame, origin, 6, (0, 255, 255), -1) # Yellow filled circle
    cv2.circle(frame, origin, 6, (0, 0, 0), 1)      # Black border for contrast
...
class RealTimeObjectTracker:
    ...
    def process_frame(self, frame_bgr):
        ...
        if self.state == "ACQUISITION":
            mask, template_idx = self._run_acquisition(frame_rgb) # Make sure _run_acquisition returns the idx!
            if mask is not None:
                clear_cuda_memory()
                self.state = "TRACKING"
                result_mask = self.seg_tracker.init_stream(frame_rgb, mask)
                
                # Unpack BOTH Rotation and Translation from the template
                self.current_R, template_t = self.template_poses[template_idx]
                print(f"{'#'*30}\nTemplate trans: {template_t}\n{'#'*30}")
            ...
            if logit_score < self.tracking_threshold or mask.sum() < 50:
                ...
                # Run Frame 1+ Optimization (Tight Bounds, locked R)
                if self.optimizer is not None and self.current_R is not None:
                    self.current_t = self.optimizer.optimize_translation_only(
                        mask, self.current_R, self.current_t, is_first_frame=False
                    )
...
#                 if pipeline.current_R is not None and pipeline.current_t is not None:
...
#                     # ---> NEW: Extract XYZ and display as text overlay
#                     tx, ty, tz = pipeline.current_t.flatten()
#                     xyz_text = f"XYZ: {tx:.3f}m, {ty:.3f}m, {tz:.3f}m"
#                     # Render below the FPS counter in yellow
#                     cv2.putText(color_image, xyz_text, (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Single-Frame SAM6D Debugging")
    parser.add_argument("--cad_path", required=True, help="Object CAD file")
    parser.add_argument("--template_dir", required=True, help="Path to object templates")
    parser.add_argument("--image_path", required=True, help="Path to the static test image")
    args = parser.parse_args()
    ...
    # ==========================================
    # 3. Setup Camera & Optimizer
    # ==========================================
    # Use the standard webcam intrinsic matrix
    K_matrix = np.array([
        [640.,   0., 320.],
        [  0., 640., 240.],
        [  0.,   0.,   1.]
    ...
    # ==========================================
    # 4. Single Frame Execution
    # ==========================================
    print(f"\n--- Processing Image: {args.image_path} ---")
    color_image = cv2.imread(args.image_path)
    ...
    if mask is not None:
        ...
        if pipeline.current_t is not None:
            tx, ty, tz = pipeline.current_t.flatten()
            print(f"Final Detected XYZ: {tx:.3f}m, {ty:.3f}m, {tz:.3f}m")
        else:
            print("Translation vector is None!")
        ...
        # Add blue tint
        color_image[bool_mask] = color_image[bool_mask] * 0.5 + np.array([255, 0, 0], dtype=np.uint8) * 0.5
        cv2.putText(color_image, f"State: {state} (Single Frame)", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)
        
        # Save output
        output_path = "debug_static_frame.jpg"
        cv2.imwrite(output_path, color_image)
        print(f"\nSUCCESS: Debug image saved to {output_path}")
        
    else:
        print("\nFAILURE: Acquisition failed. No target found by DINOv2.")