...
ONNX_PATH = "dinov2_s14_fp32.onnx"
FP16_ONNX_PATH = "dinov2_s14_fp16.onnx"
INT8_ONNX_PATH = "dinov2_s14_int8.onnx"
TRT_CACHE_DIR = "./trt_cache_dinov2"
DEVICE = 'cuda:0'
...
def export_and_quantize_dino():
    ...
    # NOTE: dynamic_axes removed to ensure perfect TensorRT compilation
    torch.onnx.export(
...
def test_inference_dino(image_path):
        ...
        },
        {
            ...
            "provider": [
                ('TensorrtExecutionProvider', {
                    'trt_engine_cache_enable': True,
                    'trt_engine_cache_path': TRT_CACHE_DIR,
                    'trt_fp16_enable': False, # Model is already FP16 native
                }),
                'CUDAExecutionProvider'
            ], 
            "input": x_fp32
    ...
    for cfg in configs:
        ...
        print(f"  -> Avg Time: {avg_time_ms:.2f} ms ({fps:.1f} FPS)")

        # --- VISUALIZE PCA ---
        patch_tokens = outputs[1]
        features = patch_tokens[0] # Shape (1369, 384)
...
if __name__ == "__main__":
    # Ensure Windows PATH has TensorRT DLLs if running on A6000!
    export_and_quantize_dino()
    test_inference_dino("image.png")