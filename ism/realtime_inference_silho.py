...
def get_depth_centroid(mask, depth_image_meters, K_matrix):
    """Calculates the metric 3D centroid of the masked depth pixels."""
    ys, xs = np.where(mask > 0)
    if len(ys) == 0: return None
    
    # Extract depth values for the mask
    depths = depth_image_meters[ys, xs]
    valid = depths > 0
    ...
    # Use median to ignore extreme noise spikes
    centroid = np.array([np.median(x), np.median(y), np.median(z)], dtype=np.float32)
    return centroid.reshape(3, 1)

def draw_6d_pose(frame, K, R, t, bbox_3d):
    rvec, _ = cv2.Rodrigues(R)
    dist_coeffs = np.zeros((4, 1)) 
    
    # PROJECT DIRECTLY IN METERS
    pts_2d, _ = cv2.projectPoints(bbox_3d, rvec, t, K, dist_coeffs)
    pts_2d = np.int32(pts_2d).reshape(-1, 2)
    
    edges = [(0, 1), (0, 2), (0, 4), (1, 3), (1, 5), (2, 3), (2, 6), (3, 7), (4, 5), (4, 6), (5, 7), (6, 7)]
    for edge in edges:
        cv2.line(frame, tuple(pts_2d[edge[0]]), tuple(pts_2d[edge[1]]), (0, 255, 0), 2)
        
    # Project Coordinate Axes (0.05 meters = 5cm)
    axis_length_m = 0.05 
    axes_3d = np.float32([[0, 0, 0], [axis_length_m, 0, 0], [0, axis_length_m, 0], [0, 0, axis_length_m]])
    axes_2d, _ = cv2.projectPoints(axes_3d, rvec, t, K, dist_coeffs)
    axes_2d = np.int32(axes_2d).reshape(-1, 2)
    origin = tuple(axes_2d[0])
    
    cv2.line(frame, origin, tuple(axes_2d[1]), (0, 0, 255), 3) 
    cv2.line(frame, origin, tuple(axes_2d[2]), (0, 255, 0), 3) 
    cv2.line(frame, origin, tuple(axes_2d[3]), (255, 0, 0), 3) 
...
class SilhouetteOptimizer:
    def __init__(self, cad_path, K_matrix, num_samples=250):
        self.K = K_matrix
        ...
        # This shifts all vertices so (0,0,0) is the dead center of the object
        mesh.vertices -= mesh.bounding_box.centroid
            
        # Get bounding box for drawing later (Now centered around 0,0,0)
        min_b, max_b = mesh.bounds
        self.bbox_3d = np.array([
            [min_b[0], min_b[1], min_b[2]], # 0
            [max_b[0], min_b[1], min_b[2]], # 1
            [min_b[0], max_b[1], min_b[2]], # 2
            [max_b[0], max_b[1], min_b[2]], # 3
            [min_b[0], min_b[1], max_b[2]], # 4
            [max_b[0], min_b[1], max_b[2]], # 5
            [min_b[0], max_b[1], max_b[2]], # 6
            [max_b[0], max_b[1], max_b[2]]  # 7
        ], dtype=np.float32)

        # Sample points on the surface for the optimizer to track
        self.model_points, _ = trimesh.sample.sample_surface(mesh, num_samples)
        self.model_points = self.model_points.astype(np.float32)

    def _project_points(self, rvec, tvec):
        """Projects 3D CAD points to 2D using current pose guess."""
        # Fast OpenCV projection
        pts_2d, _ = cv2.projectPoints(
            self.model_points, rvec, tvec, self.K, np.zeros((4, 1))
        )
        return pts_2d.reshape(-1, 2)

    def _loss_function(self, params, dist_transform, img_h, img_w):
        rvec, tvec = params[:3], params[3:]
        pts_2d, _ = cv2.projectPoints(self.model_points, rvec, tvec, self.K, np.zeros((4, 1)))
        pts_2d = pts_2d.reshape(-1, 2)
        
        # --- THE FIX: Continuous Bilinear Interpolation ---
        from scipy.ndimage import map_coordinates
        
        # map_coordinates expects (y, x) arrays
        coords = np.vstack((pts_2d[:, 1], pts_2d[:, 0]))
        
        # Evaluate the distance transform at exact float coordinates.
        # mode='constant', cval=1000.0 heavily penalizes points that fall off the screen.
        residuals = map_coordinates(dist_transform, coords, order=1, mode='constant', cval=1000.0)
            
        return residuals

    def optimize_pose(self, mask, init_R, init_t):
        ...
        # 2. Setup Initial Guess
        rvec = R_scipy.from_matrix(init_R).as_rotvec()
        init_params = np.hstack((rvec, init_t.flatten()))
        
        # 3. Allow rotation to change freely, but restrict XYZ drift
        bounds_lower = [
            -np.inf, -np.inf, -np.inf, 
            init_t[0,0] - 0.05, init_t[1,0] - 0.05, init_t[2,0] - 0.02
        ]
        bounds_upper = [
            np.inf, np.inf, np.inf, 
            init_t[0,0] + 0.05, init_t[1,0] + 0.05, init_t[2,0] + 0.15 
        ]
        
        # 4. Optimize!
        result = least_squares(
            fun=self._loss_function,
            x0=init_params, args=(dist_transform, img_h, img_w),
            bounds=(bounds_lower, bounds_upper), method='trf', max_nfev=30
        )
        
        opt_R = R_scipy.from_rotvec(result.x[:3]).as_matrix()
        opt_t = result.x[3:].reshape(3, 1)
        return opt_R, opt_t
    
    def debug_optimizer_state(self, mask, current_R, current_t):
        """
        Creates an X-Ray view of the Distance Transform and the projected CAD model.
        """
        img_h, img_w = mask.shape
        mask_uint8 = (mask * 255).astype(np.uint8)
        
        # Recreate the Gravity Field
        edges = cv2.Canny(mask_uint8, 100, 200)
        edges_inv = cv2.bitwise_not(edges)
        dist_transform = cv2.distanceTransform(edges_inv, cv2.DIST_L2, 3)
        
        # Normalize the distance transform for visualization (0-255 grayscale)
        dt_vis = cv2.normalize(dist_transform, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8)
        dt_color = cv2.cvtColor(dt_vis, cv2.COLOR_GRAY2BGR)
        
        # Project the CAD points using the CURRENT pose
        pts_2d = self._project_points(R_scipy.from_matrix(current_R).as_rotvec(), current_t)
        
        # Draw the CAD points on the gravity field
        out_of_bounds = 0
        for pt in pts_2d:
            u, v = int(round(pt[0])), int(round(pt[1]))
            if 0 <= u < img_w and 0 <= v < img_h:
                # Green dots = points successfully projected onto the screen
                cv2.circle(dt_color, (u, v), 1, (0, 255, 0), -1)
            else:
                out_of_bounds += 1
                
        # Draw the true SAM 2 mask boundary in Red
        dt_color[edges > 0] = [0, 0, 255]
        
        cv2.putText(dt_color, f"Out of bounds pts: {out_of_bounds}", (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                    
        cv2.imshow("Optimizer X-Ray", dt_color)
        cv2.waitKey(1)
...
class RealTimeObjectTracker:
    def __init__(self, seg_generator, img_descriptor, seg_tracker, device="cuda"):
        ...
        self.seg_tracker = SAM2VideoDetectorWrapper(seg_tracker)
        ...
        self.template_poses = [] 
        self.optimizer = None # We will inject this later
        self.template_images = []      # <--- NEW: Store the images for debugging
        self.current_template_img = None # <--- NEW: Store the currently active template
        self.current_R = None
        self.current_t = None

    def load_templates(self, template_dir, poses_path):
        """
        ...
        # ---> NEW: Load the BlenderProc Camera Poses <---
        print(f"Loading Camera Poses from {poses_path}...")
        cam_poses = np.load(poses_path)
        
        if len(cam_poses) < num_templates:
            raise ValueError(f"Found {num_templates} templates but only {len(cam_poses)} poses!")
            
        processed_crops = []
        self.template_poses = [] # Clear the list
        ...
        for idx in range(num_templates):
            ...
            # ---> NEW: Save a copy of the raw template image for the debug window
            self.template_images.append(cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR))
            ...
            resized_crop = F.interpolate(padded_crop.unsqueeze(0), size=(224, 224), mode='bilinear', align_corners=False)
            processed_crops.append(resized_crop.squeeze(0))
            
            # ---> NEW: Extract the correct Initial Rotation <---
            # Get the 4x4 Camera-to-World matrix for this specific template
            c2w_4x4 = cam_poses[idx]
            
            # Invert it to World-to-Camera for OpenCV
            w2c_4x4 = np.linalg.inv(c2w_4x4)
            
            # Extract the 3x3 rotation matrix and ensure it's float32
            init_R = w2c_4x4[:3, :3].astype(np.float32)
            self.template_poses.append(init_R)
    ...
    def process_frame(self, frame_bgr, depth_image_meters, K_matrix):
        frame_rgb = cv2.cvtColor(frame_bgr, cv2.COLOR_BGR2RGB)
        start_time = time.perf_counter()

        if self.state == "ACQUISITION":
            mask, template_idx = self._run_acquisition(frame_rgb)
            if mask is not None:
                self.state = "TRACKING"
                self.seg_tracker.init_stream(frame_rgb, mask)
                
                # Initialize starting pose for the optimizer
                self.current_R = self.template_poses[template_idx]
                self.current_t = get_depth_centroid(mask, depth_image_meters, K_matrix)
                # ---> NEW: Store the winning template image to display later
                self.current_template_img = self.template_images[template_idx]
            ...
            if logit_score < self.tracking_threshold or mask.sum() < 50:
                ...
                #     # 1. ALWAYS get a fresh centroid from the current frame's depth!
                #     fresh_t_guess = get_depth_centroid(mask, depth_image_meters, K_matrix)
                    
                #     if fresh_t_guess is not None:
                #         # 2. Feed the new translation and the previous rotation
                #         self.current_R, self.current_t = self.optimizer.optimize_pose(mask, self.current_R, fresh_t_guess)
                    
                #     self.optimizer.debug_optimizer_state(mask, self.current_R, self.current_t)
                ##########################################################################
                # ---> NEW: Keep updating translation so the box follows the object
                fresh_t_guess = get_depth_centroid(mask, depth_image_meters, K_matrix)
                if fresh_t_guess is not None:
                    self.current_t = fresh_t_guess
    ...
    def _run_acquisition(self, frame_rgb):
        ...
        with torch.inference_mode(), torch.autocast(device_type=self.device, dtype=torch.bfloat16):
            ...
            if not sam_outputs: return None, None
            ...
            if best_scores[winner_idx] > self.acquisition_threshold:
                ...
                # ------------------------------------------
                return sam_outputs[winner_idx]['segmentation'], best_template_idx[winner_idx].item()
...
if __name__ == "__main__":
    ...
    # ==========================================
    pipeline.load_templates(args.template_dir, "../data/cam_poses/cam_poses_level0.npy")
    ...
    pipeline.optimizer = SilhouetteOptimizer(args.cad_path, K_matrix)

    ######################## Set up Output videofile ########################
    fps_out = 30
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video_path = "../results/tracking_output.mp4"
    ...
    try:
        while True:
            ...
            mask, state, fps = pipeline.process_frame(color_image, depth_image, K_matrix)
            num_frames += 1
            # Vis
            if mask is not None:
                ...
                # Draw the pose using the pipeline's internal tracked state
                if pipeline.current_R is not None:
                    color_image = draw_6d_pose(
                        frame=color_image, 
                        K=K_matrix, 
                        R=pipeline.current_R, 
                        t=pipeline.current_t, 
                        bbox_3d=pipeline.optimizer.bbox_3d
                    )
                    
                # Optional: Add blue tint after drawing
            ...
            # # ---> NEW: Display the matched template
            # if pipeline.current_template_img is not None:
            #     cv2.imshow("DINOv2 Matched Template", pipeline.current_template_img)
